...
| 能力基线与回归收紧 | 自增益改动前后验证已知可用能力；匹配基线的失败会被归类为既有能力回归，写操作修复包停在授权等待 | `config/openclaw/capability_baseline_cases.json`、`scripts/openclaw/verify_capability_baseline.py`、`scripts/openclaw/regression_repair_runner.py`、`scripts/remote_verify_capability_baseline.py` |
