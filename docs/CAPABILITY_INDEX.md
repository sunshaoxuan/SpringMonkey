...
| 官方运行态影子迁移与失败恢复守护 | 优先读取 OpenClaw Tasks、Task audit、Doctor 与 Health，保留 journal fallback；失败形成持久 incident，逐点修复并通过官方 cron run 有界重跑，同时以 cron contract 指纹保证任务、排期和投递目标不漂移 | `scripts/openclaw/official_runtime_shadow_bridge.py`、`scripts/openclaw/cron_recovery_guard.py`、`scripts/openclaw/model_runtime_probe.py`、`config/openclaw/official_runtime_migration.json`、`docs/runtime-notes/openclaw-official-runtime-shadow-migration-2026-07.md` |
