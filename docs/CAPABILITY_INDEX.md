...
| 模型兜底策略 | `22545` Ollama/Qwen 兜底已退役；仅在显式配置新的可用兜底端点时启用。Gemini Pro 独立订阅可作为后续候选方案，接入前需要独立鉴权、最小调用和安全评估 | `scripts/openclaw/model_fallback_client.py`、`scripts/remote_install_public_model_resources.py` |
