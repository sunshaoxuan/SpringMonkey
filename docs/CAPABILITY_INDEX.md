...
| 模型兜底策略 | `22545` Ollama/Qwen 兜底已退役；Gemini Pro 通过 sub2api 的 `gemini-pro-agent` 作为 smoke-gated 显式兜底候选；不要使用本机 Cockpit sidecar 作为生产兜底 | `scripts/openclaw/model_fallback_client.py`、`scripts/remote_install_public_model_resources.py`、`scripts/remote_install_gemini_model_fallback.py` |
