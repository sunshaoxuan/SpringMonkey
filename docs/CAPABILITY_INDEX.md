...
| 聊天与任务总控主模型 | `openai-codex/gpt-5.3-codex-spark` 主力，走 sub2api；生产 URL 为 `http://ccnode.briconbric.com:49530/v1`，对应内网 `192.168.20.54:62342` | `scripts/openclaw/model_fallback_client.py`、`config/news/broadcast.json`、`docs/runtime-notes/openclaw-gpt-5.6-migration-2026-07.md` |
| 模型兜底策略 | `22545` Ollama 路径已退役；49530 Sub2API 的 `hf.co/unsloth/Qwen3.8-27B-GGUF:UD-IQ3_S` 经实时目录、chat 和完整 IntentFrame smoke 后作为通用与意图 fallback，意图层会在网络失败或无效 frame 时继续尝试，fallback 独立超时为 180 秒 | `scripts/openclaw/model_fallback_client.py`、`scripts/openclaw/harness_intent_agent.py`、`scripts/remote_install_sub2api_qwen_fallback.py` |
...
| 聊天/任务与执行深度自动判定 | 用户不用写长提示词；入口层会把寒暄保留为聊天，把真实委托判成任务，并进一步判定 `atomic / staged / agentic`，让多步任务默认接入步骤、工具、自增强流程 | `scripts/openclaw/agent_society_entry_policy.py`、`scripts/cron/upsert_generic_cron_job.py`、`docs/runtime-notes/generic-cron-task-domain-2026-04.md` |
| Job Orchestrator Wrapper | cron/pipeline job 不再只等同于一个黑盒 exec；orchestrator 会先进入 kernel session，把脚本命令当成 step action/tool，失败时写 gap、生成 helper、验证并 bounded retry | `scripts/openclaw/job_orchestrator.py`、`docs/policies/JOB_ORCHESTRATOR_EXECUTION_MODEL.md` |
| 树状执行报告与共享上下文 | kernel 现在在 intent/task/step 上记录 order、depends_on、parallel_group、shared_context_keys、context_policy，可输出树状报告；TimesCar trace 也记录步骤依赖和浏览器/login 共享上下文 | `scripts/openclaw/agent_society_kernel.py`、`scripts/openclaw/test_agent_society_tree_report.py`、`scripts/timescar/task_runtime.py` |
...
| 官方优先运行态迁移与失败恢复增益 | 官方 Cron retry/backoff/watchdog、Tasks maintenance、Doctor repair 与模型 fallback 先接管；SpringMonkey 仅在官方处理耗尽或未覆盖时形成单 job incident，逐点修复并有界重跑，以 cron contract 指纹和重跑前官方刷新防止漂移与重复执行 | `scripts/openclaw/official_runtime_shadow_bridge.py`、`scripts/openclaw/cron_recovery_guard.py`、`scripts/openclaw/model_runtime_probe.py`、`config/openclaw/official_runtime_migration.json`、`docs/runtime-notes/openclaw-official-runtime-shadow-migration-2026-07.md` |
...
| 能力基线与回归收紧 | 自增益改动前后验证已知可用能力；精确基线或同 `domain/action/safety` 能力族的失败会被归类为既有能力回归 / `known_direction_repair`，写操作回归允许自动修内部代码、测试、注册表和部署验证，真实外部写入仍受原任务或明确重试门控 | `config/openclaw/capability_baseline_cases.json`、`scripts/openclaw/verify_capability_baseline.py`、`scripts/openclaw/regression_repair_runner.py`、`scripts/remote_verify_capability_baseline.py` |
| 自增益实战验收 | 在临时 Git worktree 中构造安全合成回归，要求产生真实 diff、测试、commit、baseline 结果和长任务阶段记录；只读回归可重试，写操作回归保持外部副作用门控 | `scripts/openclaw/self_evolution_gauntlet.py`、`scripts/openclaw/verify_self_evolution_closure.py --require-gauntlet`、`scripts/remote_verify_self_evolution_gauntlet.py` |
| 通用 recurring job 触发 | owner DM 可以触发已登记、已启用、模型和投递目标校验通过的 recurring cron job；工具本身不绑定具体业务任务 | `config/openclaw/recurring_job_capabilities.json`、`scripts/openclaw/recurring_cron_run_tool.py`、`scripts/openclaw/test_recurring_cron_run_tool.py` |
...
| 回复图片/截图发送修复 | 当浏览器截图已生成但 Discord/LINE 回复前被媒体优化层丢弃（如 `dropping blocked reply media`、`Failed to optimize image`、缺少 `sharp`）时，验证并修复 OpenClaw 图片附件处理依赖 | `scripts/remote_repair_reply_media_images.py` |
...
| `scripts/remote_repair_openclaw_gateway_config.py` | 从宿主机 Git checkout 执行 Gateway 配置启动修复，重启并验证 `/healthz` |
