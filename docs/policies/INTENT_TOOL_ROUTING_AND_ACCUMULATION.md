...
| `discord_dm_control` | owner 在 Discord 私信中下达控制台指令，如 TimesCar 保留/取消/改时 | Discord Gateway 事件触发入口；运行侧必须满足 `dmPolicy` / `allowFrom` 预检，见 `scripts/remote_fix_discord_dm_event_inbound.py` | 入口层必须记录入站事件并 ack；真实订单写操作必须有专用执行器与确认页校验 |
...
运行时新增机器可读注册表：`config/openclaw/intent_tools.json`。Discord owner DM 入口应优先调用 `scripts/openclaw/intent_tool_router.py`，由该注册表决定工具命中、参数抽取、执行入口、写操作权限、验证命令与失败策略。策略表仍保留人类可读解释，但新增/修改可执行能力时必须同步机器注册表，并通过 `scripts/openclaw/verify_intent_tool_registry.py` 校验。
...
| `discord.dm_event_inbound.fix` | 修复 Discord owner DM 事件驱动入口被预检策略拦截的问题 | `scripts/remote_fix_discord_dm_event_inbound.py` | `channels.discord.dmPolicy=allowlist` 且 `channels.discord.allowFrom` 包含 owner user id；polling timer 必须不存在 |
| `discord.dm.intent_router` | Discord owner DM 通用事件路由；每条私信要么命中注册工具，要么明确 ack 并记录 capability gap | `config/openclaw/intent_tools.json` + `scripts/openclaw/intent_tool_router.py`；运行时补丁源 `scripts/openclaw/patch_discord_timescar_dm_preroute.py` | `verify_intent_tool_registry.py` 通过；补丁后 `node --check` 通过；不得静默无响应 |
| `timescar.dm.query` | TimesCar 私信查询未来 N 小时预约 | `scripts/timescar/timescar_handle_dm_adjust_request.py`，注册表 `intent_id=timescar.reservation_query` | 查询类只读；输出预约列表或明确无预约 |
| `timescar.dm.adjust_start` | TimesCar 私信改开始时间 | `scripts/timescar/timescar_handle_dm_adjust_request.py` → `timescar_adjust_reservation_window.py`，注册表 `intent_id=timescar.reservation_adjust_start` | 写操作必须有幂等账本、确认页校验、提交后回查 |
...
Discord owner DM 控制台必须走 Discord Gateway 事件触发，不得把 20 秒轮询私信频道作为正式入口。若 Discord REST API 可见私信但 OpenClaw journal 无入站记录，优先检查并修复 `dmPolicy` / `allowFrom` / Gateway event handler / preflight drop 日志；最低合格标准是：每条 owner 私信要么路由到已注册工具，要么返回“未支持/未执行”的明确 ack，不得静默无响应。
...
当前 Discord owner DM 补丁已升级为通用 intent tool router：JS 入口只负责调用 `scripts/openclaw/intent_tool_router.py` 并按意图结果分流；具体业务边界来自 `config/openclaw/intent_tools.json`。路由必须是多态：`chat` 由路由器调用公共聊天模型生成回复并直接回 DM，避免普通聊天进入长流程后卡在最终投递层；`registered_task` 调用确定性工具；未命中任务必须进入 `scripts/openclaw/dm_capability_gap_runner.py`，先写入 Agent Society kernel，再按安全分类处理。`auto_safe_readonly_gap` 可生成/复用注册表工具候选、验证并重放原始请求；`unsafe_gap` 与 `ambiguous_gap` 只能记录能力缺口和补强计划，不得自动执行写操作或不明确任务。意图判断必须是“轻量大模型基于本次消息、可用上下文与注册表工具清单优先选择 chat / registered_task / registered_candidate / auto_safe_readonly_gap / unsafe_gap / ambiguous_gap；本地注册表关键词路由只在模型不可用、返回无效或选中未知工具时兜底”。不得把所有私信都强行当任务，也不得让真实任务静默退回闲聊。
...
TimesCar “保留这单/不要取消”与“取消这单”不得误路由到改开始时间工具。保留动作只写入 `timescar_user_decisions.json`，供 24 小时取消提醒跳过该预约；取消动作在仓库具备已验证的专用取消提交器前，只能返回明确的“未执行取消”安全响应，不得假装已取消。
