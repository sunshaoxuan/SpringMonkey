...
当前 Discord owner DM 补丁已升级为通用 intent tool router：JS 入口只负责调用 `scripts/openclaw/intent_tool_router.py` 并按意图结果分流；具体业务命中来自 `config/openclaw/intent_tools.json`。路由必须是三态：`chat` 进入正常聊天回复；`registered_task` 调用确定性工具；`unsupported_task` 返回“未执行，已记录能力缺口”，并把 gap 写入 Agent Society kernel/fallback gap log。不得把所有私信都强行当任务，也不得让真实任务静默退回闲聊。
