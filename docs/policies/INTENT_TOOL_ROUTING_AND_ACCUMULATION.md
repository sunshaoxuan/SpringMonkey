...
当前 Discord owner DM 补丁已升级为通用 intent tool router：JS 入口只负责调用 `scripts/openclaw/intent_tool_router.py` 并回传结果；具体业务命中来自 `config/openclaw/intent_tools.json`。如果没有注册工具，必须返回“未执行，已记录能力缺口”，并把 gap 写入 Agent Society kernel/fallback gap log，不得继续静默等待主聊天模型。
