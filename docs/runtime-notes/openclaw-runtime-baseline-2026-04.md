...
- 主力聊天模型：`openai-codex/gpt-5.5`
- 候补模型：`ollama/qwen3:14b`
- 新闻总控：`openai-codex/gpt-5.5`
- 新闻 worker：`openai-codex/gpt-5.5`
...
- 这条基线必须与当前主模型 `openai-codex/gpt-5.5` 的真实上下文窗匹配；若降级到 Qwen/Ollama，再按 `qwen3:14b` 的较小上下文窗保守处理。
