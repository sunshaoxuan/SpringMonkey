...
The reserve baseline must still fit the active model context window; for the current `openai-codex/gpt-5.3-codex-spark` primary route use the Codex context baseline. The old `22545` Ollama/Qwen chat fallback is retired and should not drive compaction sizing.
