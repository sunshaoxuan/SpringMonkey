...
- `openclaw/agent_society_runtime_record_gap.py`：把真实 direct-task 失败写进 durable kernel，并在可复用时自动落 helper scaffold 到 `scripts/openclaw/helpers/`；当前已接通 LINE direct `no-response` 与 `auto-reply failed`
