...
- `openclaw_behavior_rule_gate.py`
  ...
  - 附加检查：`config/openclaw/intent_tools.json` 注册表必须有效；修改中的 OpenClaw JS dist 补丁必须带 `node --check` 等语法门禁。
...
- `remote_verify_capability_baseline.py`
  - 用途：只读验收第五阶段能力基线；远端验证 registry/harness、能力基线和 TimesCar 写操作回归授权门。
...
- `openclaw/recurring_cron_run_tool.py`：通用 recurring job 触发器；从 `config/openclaw/recurring_job_capabilities.json` 解析已登记任务，校验启用状态、模型和投递 owner 后再运行对应 OpenClaw cron job
...
- `timescar/timescar_handle_dm_adjust_request.py`：Discord 私信 TimesCar 指令入口；查询未来 N 小时预约，或解释明确的“明天/后天”开始时间变更，调用正式改单器并记录完成账本防重复提交
