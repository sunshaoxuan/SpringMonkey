...
def build_message(cfg: dict, job: dict) -> str:
    ...
    news_orchestrator = model_cfg.get("newsOrchestrator", model_cfg.get("name", "openai-codex/gpt-5.6-sol"))
    news_worker = model_cfg.get("newsWorker", "openai-codex/gpt-5.6-sol")
...
def build_pipeline_cron_message(cfg: dict, spec: dict) -> str:
    ...
    model_cfg = cfg.get("model", {})
    nw = model_cfg.get("newsWorker", "openai-codex/gpt-5.6-sol")
    return "\n".join(
        [
            "【新闻定时任务 · 流水线模式】",
            "",
...
def apply_job(cfg: dict, jobs_doc: dict, spec: dict):
    ...
    existing["payload"] = {
        "kind": "agentTurn",
        "message": body,
        "model": cfg["model"].get("newsOrchestrator", cfg["model"].get("name", "openai-codex/gpt-5.6-sol")),
