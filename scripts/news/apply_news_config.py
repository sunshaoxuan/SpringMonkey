...
def build_message(cfg: dict, job: dict) -> str:
    ...
    news_orchestrator = model_cfg.get("newsOrchestrator", model_cfg.get("name", "openai-codex/gpt-5.3-codex-spark"))
    news_worker = model_cfg.get("newsWorker", "openai-codex/gpt-5.3-codex-spark")
...
def _news_execution(cfg: dict) -> dict:
...
def build_pipeline_cron_message(cfg: dict, spec: dict) -> str:
    ...
    model_cfg = cfg.get("model", {})
    nw = model_cfg.get("newsWorker", "openai-codex/gpt-5.3-codex-spark")
    return "\n".join(
        [
            "【新闻定时任务 · 流水线模式】",
            "",
            ...
            "   除非宿主机重启、人工维护或用户明确要求取消，本轮新闻流水线都必须继续等待直到脚本自然结束。",
            "",
...
def apply_job(cfg: dict, jobs_doc: dict, spec: dict):
    ...
    if nex.get("mode") == "pipeline":
        body = build_pipeline_cron_message(cfg, spec)
        timeout_sec = int(
            nex.get("cronTimeoutSeconds")
            or max(int(cfg["model"].get("timeoutSeconds") or 3600), 7200)
        )
    else:
        body = build_message(cfg, spec)
        timeout_sec = int(cfg["model"]["timeoutSeconds"])
    existing["payload"] = {
        "kind": "agentTurn",
        "message": body,
        "model": cfg["model"].get("newsOrchestrator", cfg["model"].get("name", "openai-codex/gpt-5.3-codex-spark")),
