...
def build_blocker_prompt(
    *,
    ...
    system = (
        ...
        "For safe read-only missing helpers, set blocker_kind=readonly_tool_missing, autonomy_allowed=true, safety_class=auto_safe_readonly, "
        "allowed_repair_action=autonomous_readonly_repair, replay_policy=allow_after_verified_promoted. "
        "For internal self-improvement that must change this repository, registered tools, tests, schedules, prompts, config, or an owner-controlled workflow, "
        ...
        "If the request asks to perform the real external state change now, publish to a public channel now, modify third-party state, or execute a real booking/payment/deletion now, "
        "choose write_operation_request with autonomy_allowed=false unless the task is explicitly limited to internal implementation and private verification. "
