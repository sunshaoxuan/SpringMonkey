    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "",
            "执行环境约束：",
            "OpenClaw apply_patch 可能只允许修改 ~/.openclaw/workspace 内文件，不能直接 patch 持久仓库绝对路径。",
            "如需修改 repo_root 下代码，必须使用以 repo_root 为 cwd 的可验证执行方式（例如 git apply、python 小脚本或等价文件编辑），随后用 git diff/status 和测试证明真实变更。",
            "",
            "必须遵守的闭环：",
            ...
            "6. 优先在当前实现 run 内直接完成，不要调用 thread-bound subagent/session spawn；只有当前工具链明确可用时才可分派子任务。",
            "",
            "建议验证：",
