...
def build_prompt(text: str, context: str, registry: dict[str, Any]) -> list[dict[str, str]]:
    system = (
        ...
        "Example: 为什么公共频道的新闻停了 means domain=cron action=status, parameters.topic=news, and tool candidate openclaw.cron.status; do not run the news job. "
        "Example: 检查新闻定时任务状态 means domain=cron action=status, parameters.topic=news, and tool candidate openclaw.cron.status. "
        "Example: 检查天气预报定时任务状态 means domain=cron action=status, parameters.topic=weather, and tool candidate openclaw.cron.status. "
