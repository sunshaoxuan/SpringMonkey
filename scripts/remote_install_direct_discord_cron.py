...
0 9 * * * root /usr/local/lib/openclaw/direct_cron_to_discord.py --name news-digest-jst-0900 --channel-id 1483636573235843072 --timeout 5400 --command bash -lc 'set -e; OUT=$(python3 /var/lib/openclaw/repos/SpringMonkey/scripts/news/jobs/news_digest_jst_0900.py); DIR=$(echo "$OUT" | sed -n "s/^PIPELINE_OK //p" | tail -n1); test -n "$DIR"; cat "$DIR/final_broadcast.md"'
0 17 * * * root /usr/local/lib/openclaw/direct_cron_to_discord.py --name news-digest-jst-1700 --channel-id 1483636573235843072 --timeout 5400 --command bash -lc 'set -e; OUT=$(python3 /var/lib/openclaw/repos/SpringMonkey/scripts/news/jobs/news_digest_jst_1700.py); DIR=$(echo "$OUT" | sed -n "s/^PIPELINE_OK //p" | tail -n1); test -n "$DIR"; cat "$DIR/final_broadcast.md"'
