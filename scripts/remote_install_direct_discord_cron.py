...
0 7 * * * root /usr/local/lib/openclaw/direct_cron_to_discord.py --name weather-report-jst-0700 --channel-id 1483636573235843072 --timeout 1800 --run-as-openclaw --command python3 /var/lib/openclaw/repos/SpringMonkey/scripts/weather/weather_image_forecast.py
...
if [ "${RUN_DIRECT_DISCORD_SMOKE:-0}" = "1" ]; then
  python3 "${HELPER}" --name weather-report-jst-0700-smoke --channel-id "${PUBLIC_CHANNEL}" --timeout 1800 --run-as-openclaw --command python3 "${REPO}/scripts/weather/weather_image_forecast.py"
fi
