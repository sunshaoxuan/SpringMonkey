...
cd "${REPO}"
python3 -c 'from PIL import Image; assert Image.Resampling.LANCZOS'
...
0 7 * * * root /usr/local/lib/openclaw/direct_cron_to_discord.py --name weather-report-jst-0700 --channel-id 1483636573235843072 --timeout 1800 --run-as-openclaw --command env OPENCLAW_WEATHER_IMAGE_MODEL_CANDIDATES=openai/gpt-image-2.5-flare OPENCLAW_WEATHER_DATA_PROVIDERS=open-meteo,wttr OPENCLAW_WEATHER_IMAGE_TIMEOUT_MS=360000 OPENCLAW_WEATHER_IMAGE_RETRIES=3 python3 /var/lib/openclaw/repos/SpringMonkey/scripts/weather/weather_image_forecast.py
