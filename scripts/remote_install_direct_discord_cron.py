...
def should_deliver_public(name: str, message: str) -> bool:
    if name.startswith("weather-report-"):
        ...
        if not media_paths:
            return False
        return all(path.is_file() and path.name.endswith("_model.png") and path.stat().st_size >= 100000 for path in media_paths)
    return True
...
0 7 * * * root /usr/local/lib/openclaw/direct_cron_to_discord.py --name weather-report-jst-0700 --channel-id 1483636573235843072 --timeout 1800 --run-as-openclaw --command env OPENCLAW_WEATHER_IMAGE_MODEL_CANDIDATES=openai/gemini-3-pro-image,openai/gpt-image-2.5-flare,openai/gpt-image-2 OPENCLAW_WEATHER_DATA_PROVIDERS=open-meteo,wttr OPENCLAW_WEATHER_IMAGE_TIMEOUT_MS=360000 OPENCLAW_WEATHER_IMAGE_RETRIES=3 python3 /var/lib/openclaw/repos/SpringMonkey/scripts/weather/weather_image_forecast.py
...
if [ "${RUN_DIRECT_DISCORD_SMOKE:-0}" = "1" ]; then
  python3 "${HELPER}" --name weather-report-jst-0700-smoke --channel-id "${PUBLIC_CHANNEL}" --timeout 1800 --run-as-openclaw --command env OPENCLAW_WEATHER_IMAGE_MODEL_CANDIDATES=openai/gemini-3-pro-image,openai/gpt-image-2.5-flare,openai/gpt-image-2 OPENCLAW_WEATHER_DATA_PROVIDERS=open-meteo,wttr OPENCLAW_WEATHER_IMAGE_TIMEOUT_MS=360000 OPENCLAW_WEATHER_IMAGE_RETRIES=3 python3 "${REPO}/scripts/weather/weather_image_forecast.py"
fi
