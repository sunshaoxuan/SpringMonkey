...
0 9 * * * root ${HELPER} --name news-digest-jst-0900 --channel-id ${PUBLIC_CHANNEL} --timeout 5400 --command bash -lc 'set -e; OUT=$(python3 ${REPO}/scripts/news/jobs/news_digest_jst_0900.py); DIR=$(printf "%s\n" "$OUT" | sed -n "s/^PIPELINE_OK //p" | tail -n1); test -n "$DIR"; cat "$DIR/final_broadcast.md"'
0 17 * * * root ${HELPER} --name news-digest-jst-1700 --channel-id ${PUBLIC_CHANNEL} --timeout 5400 --command bash -lc 'set -e; OUT=$(python3 ${REPO}/scripts/news/jobs/news_digest_jst_1700.py); DIR=$(printf "%s\n" "$OUT" | sed -n "s/^PIPELINE_OK //p" | tail -n1); test -n "$DIR"; cat "$DIR/final_broadcast.md"'
