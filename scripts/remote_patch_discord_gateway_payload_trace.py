...
cd "$REPO"
git pull --ff-only
python3 scripts/openclaw/patch_discord_gateway_ready_diagnostics.py
python3 scripts/openclaw/patch_discord_gateway_ready_state.py
python3 scripts/openclaw/patch_discord_gateway_payload_trace.py
systemctl restart openclaw.service
sleep 35
systemctl is-active openclaw.service
journalctl -u openclaw.service --since '2 minutes ago' --no-pager | egrep -i 'discord gateway trace|springmonkey gateway ready state|discord gateway diag|logged in to discord|awaiting gateway readiness|liveness warning' | tail -180
echo DONE
"""
